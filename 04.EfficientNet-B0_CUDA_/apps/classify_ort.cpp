...
int main(int argc, char* argv[]) {
    argparse::ArgumentParser program("classify_ort");
    ...
    program.add_argument("--cuda-graphs")
        .default_value(false)
        .implicit_value(true)
    ...
    try {
        // Initialize
        LOG_INFO("Initializing ORT backend...");
        auto backend = std::make_unique<OrtBackend>();
        
        if (program.get<bool>("--fp16")) {
            backend->enable_fp16(true);
        }
        
        if (program.get<bool>("--cuda-graphs")) {
        ...
        auto preprocessor = std::make_unique<Preprocessor>(224, 224, program.get<bool>("--fp16"));
        ...
        int width, height;
        DeviceMemory<uint8_t> d_image(224 * 224 * 3);  // Max size
        ...
        // Allocate buffers
        size_t input_size = preprocessor->get_output_size();
